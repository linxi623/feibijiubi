...
import { adaptUser, adaptChannels } from '@/utils/adapter'
...
export default createStore({
    ...
    actions: {
        // 获取当前用户信息（菲比啾比后端：GET /api/users/me，返回 UserVO，经 adaptUser 适配）
        async getPersonalInfo(context) {
            ...
            if (result.data.code === 200) {
                context.commit("updateUser", adaptUser(result.data.data));
        ...
        async loadChannels(context) {
            ...
            if (!token) return; // 后端该接口需要登录，未登录时跳过
            const result = await axios.get("/api/category", {
                headers: { Authorization: "Bearer " + token },
            }).catch(() => null);
            if (!result || result.data.code !== 200 || !result.data.data) return;
            context.commit("updateChannels", adaptChannels(result.data.data));
        },

        // 退出登录（菲比啾比后端：POST /api/auth/logout，服务端把 jti 写入 Redis 黑名单）
        logout(context) {
        ...
        // 获取全部未读消息数
        // 【菲比啾比后端暂未实现消息模块，此 action 保留为空实现，红点保持为 0】
        async getMsgUnread() {
            return;
        },
