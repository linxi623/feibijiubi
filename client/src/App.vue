...
export default {
    ...
    methods: {
        // 获取频道列表（菲比啾比后端：GET /api/category，需要登录，逻辑在 store 的 loadChannels action）
        getChannels() {
            this.$store.dispatch("loadChannels");
        },

        // 【菲比啾比后端暂未实现热搜接口，保留空实现】
        async getHotSearch() {
            return;
        },
        ...
        // 获取当前用户的收藏夹列表
        // 【菲比啾比后端暂未实现收藏夹模块，保留空实现】
        async getFavorites() {
            return;
        },

        // 获取用户赞踩的评论集合
        // 【菲比啾比后端暂未实现评论模块，保留空实现】
    ...
    async created() {
        ...
        if (localStorage.getItem("teri_token")) {
            ...
            // 【菲比啾比后端暂无 IM WebSocket 服务，不再建立连接】
