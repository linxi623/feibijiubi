            ...
            <div class="info-body">
                ...
                <div class="upload-form">
                    ...
                    <div class="form-item tag">
                        ...
                        <!-- 用v-if重新渲染不留缓存 -->
                        <TagInput v-if="this.selectedVideo" style="width: 80%;" v-model:sendTag="sendTag" @updateTags="val => {form.tags = val;}"></TagInput>
                    </div>
                    <!-- 推荐标签 -->
...
export default {
    ...
    data() {
        return {
            ...
            videoName: "",  // 视频原文件名
            // ---- 菲比啾比后端 COS 直传相关 ----
            tempVideoKey: null, // 后端分配的视频临时对象 Key（投稿时作为 tempVideoKey）
            cos: null,          // COS SDK 实例
            cosTaskId: null,    // COS 上传任务 ID，用于暂停/继续/取消
            cosUploadInfo: null,  // 后端返回的直传凭证信息（bucket/region 等）
            isFailed: false,    // 是否上传失败
    ...
    methods: {
        ...
        init() {
            ...
            this.tempVideoKey = null;
            this.cos = null;
            this.cosTaskId = null;
            this.cosUploadInfo = null;
            this.isFailed = false;
            this.isPause = false;
            this.videoURL = null;
            this.videoName = "";
        ...
        async handleVideoChange(event) {
            ...
            const maxSizeInBytes = 2 * 1024 * 1024 * 1024; // 后端限制 2GB
            if (!file) {
                return;
            }
            if (file.size <= maxSizeInBytes) {
                ...
                this.initCover(file);
                this.upload();  // 开始上传（COS 直传）
            } else {
                // 文件大小超出限制
                ElMessage.error("视频太大了，特丽丽装不下呜~（最大2GB）")
            }
        },
        ...
        handlePicChange(event) {
            const file = event.target.files[0];
            const maxSizeInBytes = 2 * 1024 * 1024; // 后端封面限制 2MB
            if (!file) {
                return;
            }
            if (file.size <= maxSizeInBytes) {
        ...
        // 触发暂停/继续事件（COS SDK 支持任务级暂停与续传）
        changeState() {
            if (!this.cos || !this.cosTaskId) return;
            this.isPause = !this.isPause;
            if (this.isPause) {
                this.cos.pauseTask(this.cosTaskId);
            } else {
                this.cos.restartTask(this.cosTaskId);
            }
        },

        /**
         * 菲比啾比后端视频直传流程：
         * 1. POST /api/videos/upload-url 获取临时对象 Key + STS 临时密钥；
         * 2. 用 cos-js-sdk-v5 将视频直传到腾讯云 COS（大文件自动分块，支持暂停/续传）；
         * 3. 投稿时把返回的 tempKey 作为 tempVideoKey 提交给 POST /api/videos。
         */
        async upload() {
            ...
            let prepareRes;
            try {
                prepareRes = await this.$post("/videos/upload-url", {
                    fileName: file.name,
            ...
            if (!prepareRes || !prepareRes.data || prepareRes.data.code !== 200) {
                this.isFailed = true;
                return;
            }
            const info = prepareRes.data.data;
            this.cosUploadInfo = info;
            this.tempVideoKey = info.tempKey;

            // 2. 使用临时密钥初始化 COS SDK 并直传
            this.cos = new COS({
                getAuthorization: (options, callback) => {
                    callback({
                        TmpSecretId: info.tmpSecretId,
                        TmpSecretKey: info.tmpSecretKey,
                        SecurityToken: info.sessionToken,
            ...
            this.cos.uploadFile({
                ...
                Key: info.tempKey,
                Body: file,
                SliceSize: 10 * 1024 * 1024, // 超过 10MB 自动走分块上传
                onTaskReady: (taskId) => {
                    this.cosTaskId = taskId;
                },
                ...
                if (err) {
                    // 手动取消任务也会走到这里，此时组件已被 init() 重置，无需提示
                    if (this.selectedVideo) {
                        this.isFailed = true;
                        this.isPause = true;
                    }
        ...
        // 确认取消上传
        // 后端没有取消接口：直接取消本地 COS 上传任务即可，
        // 服务端的临时文件记录会在过期后由后端自行清理
        async cancel() {
            if (this.cos && this.cosTaskId) {
                this.cos.cancelTask(this.cosTaskId);
            }
            this.init();
        ...
        /**
         * 投稿（菲比啾比后端）：
         * 1. 把裁剪好的封面上传到 POST /api/videos/cover，拿到 tempCoverKey；
         * 2. 调用 POST /api/videos 提交投稿信息（JSON），
         *    tempVideoKey 用视频直传时后端返回的 tempKey。
         */
        async submit() {
            // 判断上传情况、表单填写情况
            ...
            if (!this.tempVideoKey) {
                ElMessage.error('视频上传信息丢失，请重新上传');
                return;
            }
            ...
            try {
                // 1. 上传封面，拿到临时封面 Key（后端限制 2MB，jpg/jpeg/png）
                const blob = await (await fetch(this.coverURL)).blob();
                const cover = new File([blob], 'cover' + Date.now() + '.jpg', { type: 'image/jpeg' });
                const coverForm = new FormData();
                coverForm.append('file', cover);
                const coverRes = await this.$post("/videos/cover", coverForm, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                        Authorization: "Bearer " + localStorage.getItem("teri_token"),
                    }
                });
                if (!coverRes.data || coverRes.data.code !== 200) {
                    ElMessage.error('封面上传失败，请稍后再试');
                    this.$store.state.isLoading = false;
                    return;
                }
                const tempCoverKey = coverRes.data.data;

                // 2. 提交投稿（JSON 请求体）
                const res = await this.$post("/videos", {
                    title: this.form.title,
                    sourceType: this.form.type,     // 1 自制 2 转载
                    visibility: 0,                  // 0 公开
                    duration: this.duration,
                    mcId: this.form.category[0].id,
                    scId: this.form.category[1].id,
                    tags: this.form.tags.join(','), // 菲比啾比的标签用逗号分隔
                    description: this.form.descr,
                    tempCoverKey: tempCoverKey,
                    tempVideoKey: this.tempVideoKey,
                }, {
                    headers: { Authorization: "Bearer " + localStorage.getItem("teri_token") }
                });
                if (res.data && res.data.code === 200) {
                    ElMessage.success('投稿成功，等待审核通过后就能和大家见面啦');
                    this.init();
                    this.$emit("changeNavBarShow", true);
                } else {
                    ElMessage.error((res.data && res.data.message) || '投稿失败，请稍后再试');
                }
            } catch {
