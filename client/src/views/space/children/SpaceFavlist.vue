<template>
    <div class="space-favlist clearfix">
        <div class="fav-empty-state" v-if="favList.length === 0">
...
export default {
    ...
    mounted() {
        ...
        if (!this.fid) {
            ...
            if (fav) {
                this.$router.replace(`/space/${this.uid}/favlist?fid=${fav.fid}`);
            }
        } else {
            this.getFavVideos();
        }
...
.favInfo-cover {
    background: url('~assets/img/feibijiubi/carousel/feibijiubi-carousel-5.jpg') 50%;
...
.small-item .cover {
    background: url('~assets/img/feibijiubi/carousel/feibijiubi-carousel-6.jpg') 50%;
    background-size: cover;
    border-radius: 4px;
    display: block;
    width: 160px;
