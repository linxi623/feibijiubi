        ...
        <div class="space-container">
            ...
            <div class="h">
                <div class="wrapper">
                    ...
                    <div class="h-inner" :style="user.bg_url ? { backgroundImage: `url('${user.bg_url}')` } : {}">
                        ...
                        <div class="h-action" v-if="!isSelf">
                            <div class="h-f-btn h-follow" :class="{ 'is-following': user.subscribed }"
                                @click="setSubscribe(!user.subscribed)">
                                {{ user.subscribed ? '已关注' : '关注' }}
                            </div>
                        ...
                        <div title="更换皮肤" class="space-theme-trigger icon" v-if="isSelf"
                    ...
                    <!-- 更换主题面板 -->
                    <div class="space-theme" :class="{'theme-show': themeShow}" v-if="isSelf">
                        ...
                        <div class="theme-panel">
                            <div class="wrapper">
                                <div class="theme-list">
                                    <div class="background-editor" v-if="backgroundSource">
                                        <div class="background-cropper-area">
                                            <CropperDefault
                                                :imgUrl="backgroundSource"
                                                :wrapWidth="backgroundWrapWidth"
                                                :wrapHeight="backgroundWrapHeight"
                                                v-model:prevWidth="backgroundPrevWidth"
                                                v-model:prevHeight="backgroundPrevHeight"
                                                v-model:prevOffsetX="backgroundPrevOffsetX"
                                                v-model:prevOffsetY="backgroundPrevOffsetY"
                                                v-model:cutOrder="backgroundCutOrder"
                                                @cut="sendBackground">
                                            </CropperDefault>
                                        </div>
                                        <div class="background-preview-wrap">
                                            <div class="background-preview">
                                                <img :src="backgroundSource" alt=""
                                                    :style="`width: ${backgroundPrevWidth}px; height: ${backgroundPrevHeight}px; transform: translate(-${backgroundPrevOffsetX}px, -${backgroundPrevOffsetY}px);`">
                                            </div>
                                            <span>最终头图预览</span>
                                        </div>
                                    </div>
                                    <div class="background-current" v-else
                                        :style="user.bg_url ? { backgroundImage: `url('${user.bg_url}')` } : {}">
                                        <span v-if="!user.bg_url">暂无背景图</span>
                                    </div>
                                    <div class="background-actions">
                                        <input ref="backgroundInput" type="file" accept="image/jpeg,image/png"
                                            class="background-file-input" @change="selectBackground">
                                        <button type="button" class="background-select-btn"
                                            @click="$refs.backgroundInput.click()">选择图片</button>
                                        <button type="button" class="background-upload-btn"
                                            :disabled="!backgroundSource || backgroundUploading"
                                            @click="uploadBackground">
                                            {{ backgroundUploading ? '上传中...' : '保存头图' }}
                                        </button>
                                        <span class="background-tip">JPG/PNG，最大 2 MB</span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    data() {
        return {
            ...
            backgroundSource: "",
            backgroundWrapWidth: 330,
            backgroundWrapHeight: 60,
            backgroundPrevWidth: 330,
            backgroundPrevHeight: 60,
            backgroundPrevOffsetX: 0,
            backgroundPrevOffsetY: 0,
            backgroundCutOrder: false,
    ...
    methods: {
        // 获取用户信息（菲比啾比后端：GET /api/users/{uid}，可选登录）
        async getUserInfo(uid) {
            const headers = {};
            if (localStorage.getItem("teri_token")) {
                headers.Authorization = "Bearer " + localStorage.getItem("teri_token");
            }
            const res = await this.$get(`/users/${uid}`, { headers });
            if (!res.data) return;
            if (!res.data.data) {
                this.$router.push('/notfound');
                return;
            }
            const user = adaptUser(res.data.data);
            if (user.state === 2) {
                this.$router.push('/notfound');  // 账号已注销
                return;
            }
            this.user = user;
            this.worksCount = user.videoCount || 0;
        },
        ...
        selectBackground(event) {
            ...
            if (!["image/jpeg", "image/png"].includes(file.type)) {
                ElMessage.warning("请选择 JPG 或 PNG 图片");
                event.target.value = "";
                return;
            }
            ...
            reader.onload = e => {
                this.backgroundSource = e.target.result;
                this.backgroundPrevWidth = this.backgroundWrapWidth;
                this.backgroundPrevHeight = this.backgroundWrapHeight;
                this.backgroundPrevOffsetX = 0;
                this.backgroundPrevOffsetY = 0;
            };
            reader.readAsDataURL(file);
        },

        async uploadBackground() {
            if (!this.backgroundSource || this.backgroundUploading) return;
            this.backgroundUploading = true;
            this.backgroundCutOrder = true;
        },

        async sendBackground(croppedImage) {
            try {
                const blob = await fetch(croppedImage).then(response => response.blob());
                const background = new File([blob], "主页背景.jpg", { type: "image/jpeg" });
                const formData = new FormData();
                formData.append("file", background);
                const res = await this.$post("/users/me/background", formData, {
                    headers: {
                ...
                if (res.data && res.data.data) {
                    this.user.bg_url = resolveMediaUrl(res.data.data);
                    this.backgroundSource = "";
                    this.$refs.backgroundInput.value = "";
                    this.themeShow = false;
                    ElMessage.success("主页背景图已更新");
                }
            } finally {
...
.h-forbid .f-wrap .f-icon {
    background-image: url('~assets/img/feibijiubi/feibijiubi-logo.png');
