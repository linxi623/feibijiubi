        ...
        <div class="large-header">
            ...
            <!-- <div class="header__banner">
                <picture class="v-img banner-img">
                    <img src="~assets/img/feibijiubi/hero-banner.jpg" alt="菲比啾比">
                </picture>
                <div class="header-banner__inner">
                    <div class="logo-box">
                        <img src="~assets/img/feibijiubi/feibijiubi-logo.png" alt="菲比啾比">
                    </div>
                </div>
            </div> -->
            <header>
                ...
                <div class="tree">
                    <img src="~assets/img/feibijiubi/feibijiubi-logo.png" class="morning" alt="">
                    <img src="~assets/img/feibijiubi/feibijiubi-logo.png" class="afternoon" alt="">
                    <img src="~assets/img/feibijiubi/feibijiubi-logo.png" class="evening" alt="">
                </div>
                <div class="header-banner__inner">
                    <div class="logo-box">
                        <img src="~assets/img/feibijiubi/feibijiubi-logo.png" alt="菲比啾比">
        ...
        <div class="main__layout">
            <div class="recommended-container">
                <div class="container">
                    ...
                    <!-- 随机推荐 -->
                    <div class="feed-card" v-for="index in loadingRandom ? 11 : randomVideos.length"
                        :key="loadingRandom ? `random-skeleton-${index}` : `random-${randomVideos[index - 1].video.vid}`">
                    ...
                    <!-- 累加推荐 -->
                    <div class="video-card" v-for="item in cumulativeVideos" :key="`feed-${item.video.vid}`">
                        <!-- 骨架屏 -->
...
import { adaptVideoItem } from '@/utils/adapter.js';
...
export default {
    ...
    methods: {
        // 请求
        // 获取顶部推荐（菲比啾比后端：GET /api/videos/feed 游标分页，取一页当作推荐位）
        async getRandomVideos() {
            ...
            try {
                ...
                if (requestId !== this.randomRequestId) return;
                if (res.data.data && res.data.data.items) {
                    const nextRandomVideos = this.uniqueVideos(res.data.data.items.map(adaptVideoItem));
                    const randomIds = new Set(nextRandomVideos.map(item => item.video.vid));
                    this.randomVideos = nextRandomVideos;
                    this.cumulativeVideos = this.cumulativeVideos.filter(
                        item => !randomIds.has(item.video.vid)
                    );
                }
            } finally {
                if (requestId === this.randomRequestId) {
                    this.loadingRandom = false;
                }
            }
        },

        // 获取瀑布流视频（菲比啾比后端：GET /api/videos/feed?cursor=&size=，使用 nextCursor 翻页）
        async getCumulativeVideos() {
            ...
            const res = await this.$get("/videos/feed", { params });
            if (res.data.data) {
                const incoming = (res.data.data.items || []).map(adaptVideoItem);
                const randomIds = new Set(this.randomVideos.map(item => item.video.vid));
                const merged = [
                    ...this.cumulativeVideos,
                    ...incoming.filter(item => !randomIds.has(item.video.vid)),
                ];
                this.cumulativeVideos = this.uniqueVideos(merged);
        ...
        uniqueVideos(videos) {
            ...
            return videos.filter(item => {
                const vid = item && item.video && item.video.vid;
                if (vid == null || seen.has(vid)) return false;
                seen.add(vid);
                return true;
            });
        },
