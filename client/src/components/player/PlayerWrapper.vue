...
export default {
    ...
    methods: {
        //////// 请求 /////////
        // 增加一个播放量（菲比啾比后端：POST /api/videos/{vid}/play-count，可选登录，游客与用户同一接口）
        // 当前用户的互动状态（点赞/投币/收藏）由视频详情接口 GET /api/videos/{vid} 返回，这里不再处理
        async increasePlay() {
            const vid = Number(this.$route.params.vid);
            const headers = {};
            if (this.$store.state.user.uid) {
                headers.Authorization = "Bearer " + localStorage.getItem("teri_token");
            }
            await this.$post(`/videos/${vid}/play-count`, null, { headers }).catch(() => {});
        },

        // 保存播放进度（菲比啾比后端：POST /api/videos/{vid}/progress?playTime=，需要登录）
        async savePlayProgress() {
            if (!this.$store.state.user.uid) return; // 游客不保存进度
            const vid = Number(this.$route.params.vid);
            const playTime = this.currentTime;
            if (!playTime || playTime <= 0) return;
            await this.$post(`/videos/${vid}/progress?playTime=${playTime}`, null, {
                headers: { Authorization: "Bearer " + localStorage.getItem("teri_token") }
            }).catch(() => {});
        },
        ...
        pauseVideo() {
            ...
            this.savePlayProgress();       // 暂停时保存播放进度
        },
    ...
    beforeUnmount() {
        this.savePlayProgress();    // 离开页面时保存播放进度
