    ...
    methods: {
        ...
        async submitLogin() {
            ...
            if (result.data.code === 200) {
                ...
                // 登录接口不再返回用户对象，改为调用 /api/users/me 获取
                await this.$store.dispatch("getPersonalInfo");
                await this.$store.dispatch("loadChannels"); // 分区列表需要登录后才能获取
                // 【菲比啾比后端暂未实现消息/IM/收藏夹/评论模块，以下初始化跳过】
        ...
        async submitRegister() {
            ...
            // 菲比啾比后端：POST /api/auth/register，请求体字段相同
