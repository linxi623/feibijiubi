<template>
    <div class="user-card">
        <div class="user-card-bg" :style="user.bg_url ? { backgroundImage: `url('${user.bg_url}')` } : {}"></div>
            ...
            <div class="info">
                ...
                <p class="social">
                    ...
                    <a :href="`/space/${user.uid}/fans/fans`" target="_blank">
                        <span class="num">{{ handleNum(localFansCount) }}</span>
                ...
                <div class="official" v-if="user.auth === 1 || user.auth === 2">
                    ...
                    <span>菲比啾比{{ user.auth === 1 ? '个人' : '机构' }}认证：{{ user.authMsg }}</span>
                </div>
                <p class="sign">{{ user.description }}</p>
                <div class="btn-box">
                    <a class="not-follow" v-if="!localSubscribed && !isSelf" @click="setSubscribe(true)">关注</a>
                    <a class="following" v-else-if="!isSelf" @click="setSubscribe(false)"
                        @mouseenter="mouseEnter = true" @mouseleave="mouseEnter = false">
                        {{ mouseEnter ? '取消关注' : '已关注' }}
                    </a>
...
export default {
    ...
    data() {
        return {
            ...
            localSubscribed: !!this.user.subscribed,
            localFansCount: this.user.fansCount || 0,
        }
    },
    computed: {
        isSelf() {
        ...
        async setSubscribe(isSet) {
            ...
            try {
                ...
                if (res.data && (res.data.code === 0 || res.data.code === 200)) {
                    this.localSubscribed = isSet;
                    this.localFansCount = Math.max(0, this.localFansCount + (isSet ? 1 : -1));
    ...
    watch: {
        'user.subscribed'(value) {
            this.localSubscribed = !!value;
        },
        'user.fansCount'(value) {
            this.localFansCount = value || 0;
        },
    },
}
</script>
