...
export default {
    ...
    methods: {
        ...
        async login() {
            ...
            // 菲比啾比后端：POST /api/auth/login，成功时 data 只返回 { token }
            ...
            if (result.data.code === 200) {
                ...
                // 登录接口不再返回用户对象，需要再调用 /api/users/me 获取用户信息并校验管理员身份
                const infoRes = await axios.get("/api/users/me", {
                    headers: {
                        Authorization: "Bearer " + result.data.data.token,
                    },
                }).catch(() => null);
                if (!infoRes || infoRes.data.code !== 200) {
                ...
                this.$store.commit("updateUser", user);    // 更新vuex中当前用户信息
                this.$store.dispatch("loadChannels");      // 登录后才能拿到分区列表
